...
PASSO 1 — Leia TODOS os arquivos da pasta de knowledge-base disponibilizada a você, **começando obrigatoriamente por "01-corpus-oficial-gamehub.md"** (fonte canônica: ordem oficial do Canvas, narrativa OFC, a escada de valor DO PRÓPRIO JOGO, padrões de qualidade e a lista dos 7 documentos em escopo). Leia "00-INDEX.md" primeiro para confirmar a lista completa, depois leia CADA arquivo de metodologia (02 a 06) antes de escrever o documento correspondente.

PASSO 2 — Leia os dois arquivos de contexto nesta pasta (diretório atual): "context-ficha.md" é a ficha completa do negócio (cadastro, respostas de onboarding, vitrine pública, equipe de Funcionários de IA contratada, sede) — a "fila_geracao_documentos.contexto_snapshot" do banco; "context-concorrentes.md" é a lista REAL de concorrentes (mesmo segmento, mesma cidade, dados do próprio jogo) para o 7º documento. Toda a documentação deve se basear NESSES dados, nunca em generalidades de mercado.

PASSO 3 — Escreva, nesta pasta (diretório atual), exatamente estes 7 arquivos markdown:
...
- **07-analise-concorrencia.md** — seguindo "06-analise-concorrencia.md" da knowledge-base: comparação com os concorrentes REAIS listados em "context-concorrentes.md" (mesmo segmento, mesma cidade). Se a lista vier vazia, escreva sobre a dinâmica regional/do segmento em geral — **nunca fabrique um nome de empresa concorrente que não esteja na lista real**.

Todos os 7 arquivos seguem a estrutura de saída E.1 do arquivo 01 (cabeçalho, corpo conforme metodologia específica, "Resumo Fácil", premissas quando aplicável, próximas ações quando aplicável) — cada arquivo de metodologia (02 a 06) especifica os desvios/adições daquele documento em particular.

PASSO 4 — Antes de considerar o trabalho concluído, rode mentalmente o checklist E.2 do arquivo 01 contra os 7 arquivos escritos. Se algum item falhar, corrija antes de terminar.

Regras inegociáveis: nunca invente números financeiros ou de mercado que a ficha não forneceu (estimativas sempre rotuladas "[HIPÓTESE]" com a lógica exposta); nunca misture moeda virtual do jogo (🪙/XP) com R$ real; use a escada de valor DO GAMEHUB (D.3), nunca uma genérica; nunca invente concorrente que não esteja em "context-concorrentes.md"; toda recomendação deve ser acionável e rastreável a um dado real da ficha; se os dados da ficha forem inconsistentes entre si, aponte isso explicitamente no resumo executivo — não esconda a inconsistência.

Ao final, não escreva nenhum resumo ou explicação para mim — apenas garanta que os 7 arquivos foram escritos corretamente na pasta.`;
}
