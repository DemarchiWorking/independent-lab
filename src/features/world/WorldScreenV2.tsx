...
/**
 * `WorldScreenV2` — irmã de `WorldScreen.tsx`, não uma variante dela por
 * flag: mesma regra de projeto já usada para `VisitaScreen.tsx`
 * (`docs/world/VISITAR-VIZINHO.md` §4 — "sibling component" em vez de prop
 * `if` espalhado por um componente grande).
 *
 * Única diferença de verdade: passa `DESENHADORES_V2`
 * (`render/desenhoV2.ts`) para o `WorldCanvas` em vez do desenho padrão, e
 * preenche `itemId`/`cargoId` em `EstadoCena` — os dois campos opcionais que
 * a Fase 1 da arquitetura visual adicionou a `MovelNaCena`/`AvatarNaCena`
 * (ver `docs/architecture/ARQUITETURA-VISUAL-ITENS-SEDE.md`). Toda regra de
 * negócio (compra, mover, evoluir sede) é a MESMA — reaproveitada, não
 * reimplementada.
 *
 * Rota: `/world/v2` (`WorldScreen.tsx`/`/world` continuam 100% intocados).
 */
  ...
  const estadoCena: EstadoCena = useMemo(() => {
    ...
    return {
      ...
      moveis: moveisPosicionados.map((m) => ({
        ...
        // chave da silhueta V2 — ver render/desenhoV2.ts.desenharMovelV2
        itemId: m.item.id,
        selecionado:
      ...
      avatares: [
        ...
        ...funcionarios.map((cargoId, i) => {
          ...
          return {
            ...
            dono: false,
            // chave do adorno V2 — ver render/desenhoV2.ts.desenharAvatarV2
            cargoId,
          };
        }),
      ],
        ...
        <div className="overflow-hidden rounded-md bg-gradient-to-b from-[#0b1830] to-[#132241] p-3">
          <WorldCanvas
            ...
            onAvatarParou={(celula) =>
              setProximos(avataresProximos(celula, estadoCena.avatares, "dono"))
            }
            avatarDonoId="dono"
            onCliqueCelula={aoClicarCelula}
          />
          <p className="mt-2 text-center text-[10px] text-muted">
            Clique no chão para andar · chegue perto de alguém para interagir
          </p>

          <div className="mt-2">
            <InteracaoNpc proximos={proximos} minhaSede nivelPorCargo={nivelPorCargo} />
          </div>
